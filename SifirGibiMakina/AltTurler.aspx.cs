...
{
    public partial class AltTurler : System.Web.UI.Page
    {
        int markaID;
        int turID;
        int altturID;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            int AltTur = Convert.ToInt32(RouteData.Values["altturid"]);
            string Lang = Session["Lang"].ToString();

            var sorgu = from c in nt.tbl_MakinaAltTurleri
                        join D in nt.tbl_Diller on c.dil_ID equals D.dil_ID
                        where c.Alttur_ID == AltTur && c.Durum == true && c.dil_ID == 1
                        select new { c.tur_ID, c.Kategori, c.SeoDescription, c.SeoKeywords, c.Alttur_ID, c.FooterAciklamaSEO };
            ...
            if (!IsPostBack)
            {
                ...
                var AltTurler = from c in nt.tbl_MakinaAltTurleri
                                where c.Durum == true && c.dil_ID == 1 && c.Alttur_ID == AltTur
                                orderby c.Kategori ascending
                                select c;
        ...
        {
            int Tur = Convert.ToInt32(RouteData.Values["altturid"]);


            db_SifirGibiMakinaEntities nt = new db_SifirGibiMakinaEntities();
            string Lang = Session["Lang"].ToString();

            ddTurlerAlt.SelectedValue = Tur.ToString();
            ddTurlerAltMobil.SelectedValue = Tur.ToString();

            // Türü Seçiyoruz
            var tursorgu = from c in nt.tbl_MakinaAltTurleri
                           join T in nt.tbl_MakinaTurleri on c.tur_ID equals T.tur_ID
                           where c.Durum == true && c.Alttur_ID == Tur
                           orderby c.Kategori ascending
                           select new { c.tur_ID };
            foreach (var prod in tursorgu)
            {
                ddTurler.SelectedValue = prod.tur_ID.ToString();
                ddTurlerMobil.SelectedValue = prod.tur_ID.ToString();
            }
            ...
            var urunsorgu = from c in nt.tbl_Makinalar
                            ...
                            where c.Durum == true && c.dil_ID == 1 && c.Yayinda == true && c.Alttur_ID == Tur && (c.Statu == 2 || c.Statu == 4 || c.Statu == 5)
        ...
        protected void btnAra_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var urunsorgu = from c in nt.tbl_Makinalar
                                ...
                                orderby c.Kayit_Tarihi descending
                                select new { c.Aciklama, c.Baslik, c.Fotograf, c.makina_ID, c.Kayit_Tarihi, c.Fiyat, c.KM, c.Makina_Order, Yil = Y.Kategori, Tur = T.Kategori, Marka = M.Kategori, c.Model, c.Para_Birimi, c.marka_ID, c.tur_ID, c.yil_ID, c.Ihale, c.StokGelis_Tarihi, c.Statu, c.IlanNo, c.Yayinlanma_Tarihi, c.Alttur_ID, c.FiyatGosterilmesin, c.SEOUrl };
