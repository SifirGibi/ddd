...
{
    public partial class KategorilerList2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (RouteData.Values["altturid"] != null)
            {
                //Gelen Değer int mi kontrol ediyoruz.
                if (!Int32.TryParse(RouteData.Values["altturid"].ToString(), out int sayi))
                {
                ...
                {
                    int altturid = Convert.ToInt32(RouteData.Values["altturid"].ToString());
                    ...
                    var sorgu = from c in nt.tbl_MakinaAltTurleri
                                ...
                                select new { c.Kategori, c.FooterAciklamaSEO };
                    if (sorgu.Count() > 0)
                    {
                        ...
                        //Kategoriler
                        KategorileriDoldur(altturid);
                        YeniEklenenleriDoldur(altturid);
                        TumKategorileriDoldur(altturid);
                    }
                    else
                    {
                        Response.Redirect("/404.aspx");
        ...
        protected void KategorileriDoldur(int gelenKategori)
        {
            ...
            where log.Durum == true && s.Durum == true && s.Yayinda == true && (s.Statu == 2 || s.Statu == 5) && s.Alttur_ID == gelenKategori
            group log by new { log.Alttur2_ID, log.Kategori })
            select new { Kategori = x.Key.Kategori, Total = x.Count(), x.Key.Alttur2_ID, })
                                 orderby y.Kategori ascending, y.Total descending
        ...
        protected void YeniEklenenleriDoldur(int gelenKategori)
        {
            db_SifirGibiMakinaEntities nt = new db_SifirGibiMakinaEntities();
            string Lang = Session["Lang"].ToString();
            ...
            var urunsorgu = from c in nt.tbl_Makinalar
                            ...
                            join T in nt.tbl_MakinaTurleri on c.tur_ID equals T.tur_ID
                            join M in nt.tbl_MakinaMarkalari on c.marka_ID equals M.marka_ID
                            where c.Durum == true && c.Yayinda == true && (c.Statu == 2 || c.Statu == 5) && c.Alttur2_ID == gelenKategori
