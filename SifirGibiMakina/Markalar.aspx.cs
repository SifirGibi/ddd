...
namespace SifirGibiMakina
{
    public partial class Markalar : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            int Marka = Convert.ToInt32(RouteData.Values["markaid"]);
            string Lang = Session["Lang"].ToString();

            var sorgu = from c in nt.tbl_MakinaMarkalari
                        join D in nt.tbl_Diller on c.dil_ID equals D.dil_ID
                        where c.marka_ID == Marka && c.Durum == true && c.dil_ID == 1
                        select new { c.marka_ID, c.Kategori, c.SeoDescription, c.SeoKeywords };
            foreach (var prod in sorgu)
            {

                ///////////////////////////////////////SEO Gönderimi//////////////////////////////////////

                Master.Page.Title = prod.Kategori + " | " + WebConfigurationManager.AppSettings["MikroSiteAdi"];
                Master.Page.MetaDescription = prod.SeoDescription.ToString();
                Master.Page.MetaKeywords = prod.SeoKeywords.ToString();
                lblMarkaH1.Text = prod.Kategori + " Marka 2. El Makineler";
                lblMarkaDesc.Text = prod.SeoDescription + " İkinci el <strong><u>" + prod.Kategori + "</u></strong> marka, makine alım ve satım. Tüm 2. el <strong>" + prod.Kategori + "</strong> marka makineler. 2.el makine kategorilerinde makine satıcılarıdan ikinci el satılık makine ilanları, sahibinden ikinci el satılık makine ilanları ve ilan detaylarında ikinci el makine fiyatları bulunmaktadır.";
                /////////////////////////////////////////SEO Gönderimi/////////////////////////////////////
            }
            if (!IsPostBack)
                {
        ...
        {
            int Marka = Convert.ToInt32(RouteData.Values["markaid"]);
            ddMarkalar.SelectedValue = Marka.ToString();
            ddMarkalarMobil.SelectedValue = Marka.ToString();
            ...
            var urunsorgu = from c in nt.tbl_Makinalar
                            ...
                            where c.Durum == true && c.dil_ID == 1 && c.Yayinda == true && M.marka_ID==Marka && (c.Statu == 2 || c.Statu == 5)
        ...
        protected void ddMarkalar_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int Marka = Convert.ToInt32(ddMarkalar.SelectedItem.Value);
            var sorgu = from c in nt.tbl_MakinaMarkalari
                        where c.marka_ID == Marka && c.Durum == true
                        select new { c.marka_ID, c.Kategori, c.SeoDescription, c.SeoKeywords };
            foreach (var prod in sorgu)
            {
